<template>
  <div v-show="submitTabStatus == 'list'" class="solution-container border-b border-x-0 mt-4 border-t-0 border-solid border-gray-200 pb-4">
    <a-table
      :columns="columns"
      :data="dataList"
    ...
    >
      ...
      <template #status="{ record }">
        <div :class="[statusColor[record.judgeInfo.message] ? statusColor[record.judgeInfo.message] : 'text-red-700']">
      ...
      <template #time="{ record }">
        <div v-if="record.judgeInfo.time !== 0 && record.judgeInfo.time !== null">{{ record.judgeInfo.time }}ms</div>
        <div v-else>N/A</div>
      </template>

      <template #language="{ record }">
        {{ languageMapToKey[record.language] }}
      </template>
    </a-table>
  </div>
  <div class="solution-show" v-show="submitTabStatus == 'item'">
    <a-card :header-style="{ 'background-color': '#f2f3f5' }">
      <template #title>
        <div class="flex items-center cursor-pointer text-gray-400 hover:text-gray-700 text-sm duration-200" @click="submitTabStatus = 'list'">
      ...
      <div>
        <a-row>
          ...
          <a-col :span="12">
            <div>
              结果：
              <span :class="[statusColor[currentSubmit?.judgeInfo.message] ? statusColor[currentSubmit?.judgeInfo.message] : 'text-red-700']">
        ...
        <a-row class="mt-4">
          <a-col :span="12">
            <div>
              通过了 {{ currentSubmit?.judgeInfo?.pass_num }}/{{ currentSubmit?.judgeInfo?.total_case }}
              个数据
            </div>
          </a-col>
          <a-col :span="12">
            <div class="flex items-center">
              <span>运行时间：</span>
              <div v-if="currentSubmit?.judgeInfo.time !== 0 && currentSubmit?.judgeInfo.time !== null">{{ currentSubmit?.judgeInfo.time }} ms</div>
              <div v-else>N/A</div>
            </div>
          </a-col>
        </a-row>
...
import { QuestionControllerService, QuestionQueryRequest, QuestionSubmit, QuestionSubmitVO } from '../../generated/question';
import { languageMap, languageMapToKey } from '@/utils/common';
interface Props {
